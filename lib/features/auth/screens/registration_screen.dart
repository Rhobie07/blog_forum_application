...
class _RegistrationScreenState extends State<RegistrationScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return StoreConnector<AppState, RegisterViewModel>(
      converter: (store) => RegisterViewModel(
        loading: store.state.auth.loading,
        error: store.state.auth.error,
        emailConfirmationRequired: store.state.auth.emailConfirmationRequired,
        onRegister: _register,
        navigateToLogin: _goLogin,
      ),
      builder: (context, viewModel) {
        if (viewModel.emailConfirmationRequired && !_navigated) {
          _navigated = true;
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (!mounted) return;
